...
export default function Create({ seasons }: PageProps) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        code: '',
        duration_minutes: 60,
        total_questions_to_display: 50,
        pass_mark: 50,
        instructions: '',
        is_active: true,
        allocation_criteria: '' as any, // Storing as JSON string for submission, or handling explicitly
        allocation_type: 'all', // 'all', 'base_combo', 'department'
        department_name: ''
    });

    const selectedSeason = seasons.find(s => s.id.toString() === data.exam_season_id);
    ...
    const submit = (e: FormEvent) => {
        ...
        if (isCombinedMode) {
            ...
            } else if (data.allocation_type === 'department' && data.department_name) {
                criteria = { department_specific: true, departments: [data.department_name] };
            }
        } else if (data.allocation_type === 'department' && data.department_name) {
             // For per-subject mode, standard criteria matching
             criteria = { department: data.department_name };
        }
        
        setData('allocation_criteria', criteria ? JSON.stringify(criteria) : null);

        // We use a timeout to let setData propagate before posting, or we can just send the payload directly.
        // Inertia post uses the state at time of calling, so we pass an overridden payload.
        post('/admin/subjects', {
            data: {
                ...data,
            ...
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4 max-w-4xl mx-auto w-full">
                <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <p className="text-muted-foreground">Create a new subject and configure exam settings.</p>
                    </div>
                </div>
                            ...
                            <div className="space-y-4 rounded-lg border p-4 bg-muted/10">
                                <div className="space-y-2">
                                    ...
                                    <Select 
                                        ...
                                        onValueChange={(val) => setData('allocation_type', val)}
                                    >
                                        <SelectTrigger>
                                            <SelectValue placeholder="Select Allocation Type" />
                                        </SelectTrigger>
                                        <SelectContent>
                                            {!isCombinedMode && <SelectItem value="all">All Candidates</SelectItem>}
                                            {isCombinedMode && <SelectItem value="base_combo">Base Subject (All Candidates in Combo)</SelectItem>}
                                ...
                                {data.allocation_type === 'department' && (
                                    <div className="space-y-2 pt-2 border-t">
                                        <Label htmlFor="department_name">Department Name <span className="text-destructive">*</span></Label>
                                        <Input
                                            id="department_name"
                                        ...
                                        />
                                        <p className="text-sm text-muted-foreground">Only candidates from this department will be allocated this subject.</p>
                                    </div>
                                )}
                            </div>
